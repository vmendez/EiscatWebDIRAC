...
class Compiler(object):
  ...
  def run( self, appNameTarget=None ):
    ...
    for staticPath in self.__staticPaths:
      ...
      for extName in self.__extensions:
        ...
        if appNameTarget is None:
          ...
          gLogger.notice( "Apllication target %s, path %s, list path applications %s" % ( appNameTarget, extPath, os.listdir( extPath ) ) )
          if appNameTarget in os.listdir( extPath ):
            expectedJS = os.path.join( extPath, appNameTarget, "classes", "%s.js" % appNameTarget )
            gLogger.notice( "expectedJS %s" % ( expectedJS ) )
            if not os.path.isfile( expectedJS ):
              continue
            classPath = self.__getClasspath(extName,appNameTarget)
            gLogger.notice( "Trying to compile %s.%s.classes.%s CLASSPATH=%s" % ( extName, appNameTarget, appNameTarget, classPath ) )
            result = self.__compileApp( extPath, extName, appNameTarget, classPath )
            if not result[ 'OK' ]:
              return result
          else:
            continue
